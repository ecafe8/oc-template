...
import { type ApprovalAgentMessage, createApprovalAgent } from "../agents/approvalAgent/createApprovalAgent";

type ApprovalRequestBody = {
  messages?: unknown;
};

export const aiRoutes = new Hono<AppEnv>().post("/agents/approval", describeRoute(honoDescribeRoute("AI")), async (c) => {
  ...
  const userId = c.get("userId") ?? "test-123";

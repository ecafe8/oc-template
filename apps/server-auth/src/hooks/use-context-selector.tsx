...
export const createContextWithSelectors = <Value,>(
  defaultValue: Value
): Context<Value> => {
  ...
  const NewProvider: CallSignature<Provider<Value>> = (props: {
    value: Value;
...
export const useContextSelector = <Value, Slice>(
  ...
  const getSnapshot = (): Slice => selector(store.value);
  return useSyncExternalStore<Slice>(
    store.subscribe,
    getSnapshot, // getClientSnapshot
    getSnapshot // getServerSnapshot - 使用相同的函数确保客户端和服务端一致
  );
};

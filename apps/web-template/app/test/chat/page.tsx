...
import {
  ...
  MessageContent,
  MessageResponse,
} from "@repo/share-ui/components/ai-elements/message";
import {
  PromptInput,
  PromptInputButton,
  PromptInputFooter,
...
import { Source, Sources, SourcesContent, SourcesTrigger } from "@repo/share-ui/components/ai-elements/sources";
...
import {
  ...
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "@repo/share-ui/components/reui/dropdown-menu";
  ...
  const addUserMessage = useCallback(
    (content: string) => {
      ...
      setTimeout(() => {
        ...
        streamMessageResponse(assistantMessageKey, assistantMessageId, randomMessageResponse, reasoning);
      }, 500);
    },
    [streamMessageResponse],
  );
  ...
  return (
    <div className="relative flex size-full flex-col divide-y overflow-hidden">
                ...
                {versions.map((version) => (
                  <Message from={message.from} key={`${message.key}-${version.id}`}>
                    <div>
                      ...
                      {message.reasoning && (
                        <Reasoning duration={message.reasoning.duration} isStreaming={message.isReasoningStreaming}>
                          <ReasoningTrigger />
                          <ReasoningContent>{message.reasoning.content}</ReasoningContent>
                        </Reasoning>
                      )}
                      {(message.from === "user" || message.isReasoningComplete || !message.reasoning) && (
                        <MessageContent
                          className={cn(
      ...
      <div className="grid shrink-0 gap-4 p-4">
        <PromptInput className="divide-y-0 rounded-[28px]" onSubmit={handleSubmit}>
          <PromptInputTextarea
            ...
            <PromptInputTools>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <PromptInputButton className="!rounded-full border font-medium" variant="outline">
